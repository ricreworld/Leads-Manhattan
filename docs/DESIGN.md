...
| D-033 | 2026-07-09 | Port Authority of NY & NJ added as an overnight bid source (`paBidsScan` in scan.js): scrapes the three Solicitations Advertised boards on panynj.gov (Goods & Services, Construction, Professional Services) with browser-like headers, keyword-filters refuse/recycling/environmental ads, and routes by facility hint — NJ-side names → North Jersey bids, NY-side names → New York bids, unhinted → both (bi-state agency) | Ricardo found "Refuse Removal, Disposal and Recycling Service For John F. Kennedy International Airport - Two (2) Year Contract" — that title format is PANYNJ's advertised-solicitation convention, and PA contracts never appear on NYSCR (bi-state agency, exempt) or NJSTART, so the tool was blind to JFK/LGA/EWR/port refuse and remediation contracts. Bonfire (the PA's e-bid portal) is Cloudflare-walled, so the panynj.gov ad pages are the scrape target and Bonfire is named in the card copy as the bidding path | 🔶 first live run will verify the page parses; both parse passes (anchors, stripped lines) are tolerant and failures land in the errors list |

| D-034 | 2026-07-09 | PANYNJ counterpart authority boards wired for the remaining territories via a config-driven `authorityBidsScan()` sharing one tolerant parser (`parseBoard`/`fetchBoard`) with the PANYNJ scan: DRPA/PATCO + South Jersey Ports + SJTA → South Jersey bids, CT Airport Authority (Bradley) → Connecticut bids, Massport (goods/services + capital boards) → Massachusetts bids. Keyword filter widened with deicing/glycol/triturator/sanitary-sewer/soil (airport-specific waste streams) | Ricardo asked whether the JFK-style blind spot had counterparts in the other territories — it does: these authorities are exempt from NJSTART/CTsource/COMMBUYS, so their refuse and remediation contracts were invisible. Bid Express (SJTA/Massport submissions) and ProcureWare (CAA documents) are registration-gated, so the authorities' own public listing pages are the scrape targets and the card copy names the submission portal | 🔶 parse verified against mock boards; first live run confirms real HTML |
